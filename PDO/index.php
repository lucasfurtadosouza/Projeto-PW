<!DOCTYPE html>
<html lang="pt-BR">

<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <?php include '_parts/_linkCSS.php'; ?>
    <title>Página Inicial</title>
</head>

<body>
    <header>

    </header>
    <?php include_once '_parts/_header.php'; ?>
    <div class="p-5 text-center bg-white" style="margin-top: 58px;">

    </div>
    <?php include '_parts/_linkJS.php'; ?>

    <div class="position-relative">
        <div class="position-absolute top-50 start-50 translate-middle">
            <div class="row">
                <div class="col-sm-6">
                    <div class="card">
                    <div class="card-body">
                        <h5 class="card-title">Vídeo Final de Redes</h5>
                        <p class="card-text">Clique aqui para assistir ao vídeo final do trabalho da matéria de Redes.</p>
                        <a href="https://youtu.be/4U6mJthgESU" target="_blank" class="btn btn-primary">Assistir Vídeo</a>
                    </div>
                </div>
                </div>
                    <div class="col-sm-6">
                        <div class="card">
                        <div class="card-body">
                            <h5 class="card-title">Arquivos do Trabalho</h5>
                            <p class="card-text">Clique aqui para acessar os arquivos relacionados ao trabalho.</p>
                            <a href="https://drive.google.com/drive/folders/1haK9CfRLZaCFTyDoePCmMWqugvxVZpQh" target="_blank" class="btn btn-primary">Google Drive</a>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</body>

</html>